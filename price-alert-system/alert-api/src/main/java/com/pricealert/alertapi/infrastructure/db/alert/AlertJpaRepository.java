...
public interface AlertJpaRepository extends JpaRepository<AlertEntity, String> {
    ...
    @Query("""
            ...
            """)
    Page<AlertEntity> findByUserIdAndOptionalFilters(String userId, AlertStatus status, String symbol, Pageable pageable);

    List<AlertEntity> findBySymbolAndStatus(String symbol, AlertStatus status);

    @Modifying
    @Query("UPDATE AlertEntity a SET a.status = :newStatus, a.updatedAt = CURRENT_TIMESTAMP WHERE a.status = :currentStatus")
